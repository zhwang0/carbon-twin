from data.data_loader import Dataset_MTS, Dataset_DeepED
...
class Exp_crossformer_deeped(Exp_Basic):
    def __init__(self, args):
        super(Exp_crossformer_deeped, self).__init__(args)
    
    def _build_model(self):        
        model = Crossformer_deeped(
            self.args.data_dim_int, 
            self.args.data_dim_out, 
            self.args.in_len, 
            self.args.lab_len, 
            self.args.out_len,
            self.args.seg_len,
    ...
    def _get_data(self, flag):
        ...
        data_set = Dataset_DeepED(
            ...
            aei = args.aei,
            flag=flag,
            size=[args.in_len, args.lab_len, args.out_len],
        )
    ...
    def _process_one_batch(self, dataset_object, batch_x, batch_y, inverse = False):
        ...
        # decoder input
        ## pad time domain
        pad_len = self.args.seg_len - self.args.lab_len
        dec_inp_pad0 = torch.zeros([batch_y.shape[0], pad_len, batch_y.shape[-1]]).float()
        dec_inp = torch.cat([batch_y[:,:self.args.lab_len,:], dec_inp_pad0], dim=1).float()

        ## pad feature domain
        dec_inp_pad1 = torch.zeros([dec_inp.shape[0], dec_inp.shape[1], batch_x.shape[-1]-batch_y.shape[-1]]).float()
        dec_inp = torch.cat([dec_inp, dec_inp_pad1], dim=-1).float().to(self.device)

        outputs = self.model(batch_x, dec_inp)
        print('shape from process_one_batch', outputs.shape)
        
        if inverse:
            outputs = dataset_object.inverse_transform(outputs)
            batch_y = dataset_object.inverse_transform(batch_y)

        return outputs, batch_y[:, self.args.lab_len:self.args.lab_len+self.args.out_len].to(self.device)

    
    def predict(self, setting, load = True, inverse = False):
