...
class Crossformer_deeped(nn.Module):
    def __init__(self, data_dim_int, data_dim_out, in_len, lab_len, out_len, seg_len, win_size = 4,
                factor=10, d_model=512, d_ff = 1024, n_heads=8, e_layers=3, 
                dropout=0.0, baseline = False, device=torch.device('cuda:0')):
        super(Crossformer_deeped, self).__init__()
        self.data_dim_int = data_dim_int
        ...
        self.enc_pos_embedding = nn.Parameter(torch.randn(1, data_dim_int, (self.pad_in_len // seg_len), d_model))
        ...
        self.dec_pos_embedding = nn.Parameter(torch.randn(1, data_dim_int, (self.pad_out_len // seg_len), d_model))
    ...
    def forward(self, x_seq, x_dec):
        ...
        x_dec = self.dec_value_embedding(x_dec)
        x_dec += self.dec_pos_embedding
        x_dec = self.dec_pre_norm(x_dec)
        
        predict_y = self.decoder(x_dec, enc_out)
        print('shape from crossformer', predict_y.shape)

        return base + predict_y[:, self.lab_len:self.lab_len+self.out_len, :self.data_dim_out]
