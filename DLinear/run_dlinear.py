...
from exp.exp_main import Exp_Main, Exp_Deeped
import random
import numpy as np
import torch
...
args.model = 'DLinear'  # model name, options: [Autoformer, Informer, Transformer]

# data loader
args.data = 'deeped'  # dataset type
...
# forecasting task
args.seq_len = 12  # input sequence length
...
args.patch_len = 4  # patch length
args.stride = 2  # stride
args.padding_patch = 'end'  # None: None; end: padding on the end
args.revin = 1  # RevIN; True 1 False 0
args.affine = 0  # RevIN-affine; True 1 False 0
args.subtract_last = 0  # 0: subtract mean; 1: subtract last
args.decomposition = 0  # decomposition to trend and residual; True 1 False 0
args.kernel_size = 25  # decomposition-kernel
args.individual = 0  # individual head; True 1 False 0

# Formers
args.embed_type = 3  # 0: default 1: value embedding + temporal embedding + positional embedding 2: value embedding + temporal embedding 3: value embedding + positional embedding 4: value embedding
...
args.train_epochs = 60  # train epochs
args.batch_size = 6000  # batch size of train input data
...
args.devices = '1'  # device ids of multiple gpus
...
setting = '{}_{}_{}_ft{}_sl{}_ll{}_pl{}_dm{}_nh{}_el{}_dl{}_df{}_fc{}_eb{}_dt{}_{}_{}_noise{}'.format(
    args.model_id,
    args.model,
    args.data,
    args.features,
...
#     # encoder - decoder
#     print(batch_x.shape, dec_inp.shape)
#     outputs = exp.model(batch_x, dec_inp)
#     print(outputs.shape)
...
def eval_dlinear(emulator, path_test, path_out, age_tri, data_stat, age_used_idx, isPred2Input=True, isTrain=False, isOutput=True, isDup=True, y_lim=[0,3.5]):
  ...
  for cur_age in tqdm(range(tri_age.size)):
    ...
    for i in range(x_shape[1]):
      ...
      batch_x = torch.tensor(tmp_x).float().to(emulator.device)
      batch_y = torch.tensor(tmp_y).float()
      dec_inp = torch.zeros_like(batch_y[:, -emulator.args.pred_len:, :]).float()
      dec_inp = torch.cat([batch_y[:, :emulator.args.label_len, :], dec_inp], dim=1).float().to(emulator.device)
      cur_pred = emulator.model(batch_x, dec_inp)
      cur_pred = cur_pred[:,emulator.args.label_len:]
