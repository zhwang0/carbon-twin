...
n_epoch = 60
initial_epoch = 0
finetune_start_epoch = 50 # 0 means no finetune
...
def trainBatch(MODEL, MODEL_OPZ, ls_ckpmg, train_batch, val_batch, total_epoch, initial_epoch, y_weight, year_len=1, add_noise=False, noise_std=1e-4, early_stop=0, early_stop_patience=3):
  '''
...
MODEL = MM_LSTM_Age_v2(n_out=N_OUT, n_cons=N_CONS)
  ...
  if initial_epoch<finetune_start_epoch:
    ...
    if pass_state:
      ...
      hist_train = trainBatch(MODEL, MODEL_OPTZ, [MODEL_CKP_MANAGER], 
        ...
        add_noise=add_noise, noise_std=noise_std, early_stop=1)
